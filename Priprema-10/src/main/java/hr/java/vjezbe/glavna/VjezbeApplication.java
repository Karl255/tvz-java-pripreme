...
public class VjezbeApplication extends Application {
	...
	public void start(Stage stage) throws IOException {
		...
		var datumRodjenjaTimeline = new Timeline(
			new KeyFrame(Duration.seconds(10), e -> Platform.runLater(new DatumRodjenjaNit()))
		);
		
		datumRodjenjaTimeline.setCycleCount(Timeline.INDEFINITE);
		datumRodjenjaTimeline.play();
		
		var najboljiStudentTimeline = new Timeline(
			new KeyFrame(Duration.seconds(3), e -> Platform.runLater(new NajboljiStudentNit(stage)))
		);
		
		najboljiStudentTimeline.setCycleCount(Timeline.INDEFINITE);
		najboljiStudentTimeline.play();
	}
