...
public class Glavna {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < brojObrazovnihUstanova; i++) {
			try {
				sveuciliste.dodajObrazovnuUstanovu(nextObrazovnaUstanova(scanner));
			} catch (Exception e) {
		...
		for (var ustanova : sveuciliste.getObrazovneUstanove()) {
	...
	private static void ispisiIspiteSOcjenomIzvrstan(ObrazovnaUstanova ustanova) {
		...
		if (ispiti.size() == 0) {
			...
			ispiti.forEach(i -> System.out.printf("%s %s, %s%n", i.getStudent().getIme(), i.getStudent().getPrezime(), i.getPredmet().getNaziv()));
		}
	}
